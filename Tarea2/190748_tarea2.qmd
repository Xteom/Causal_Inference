...
$$
# falta reflexion de incisos

En ambas gráficas podemos ver como los errores de las regresiónes, no son homocedásticos, entre estratos y según como va cambiando la variable de estímulo, podemos ver como cambian los errores. (deberíamos usar errores heterocedásticos)

## Análisis
```{r}
enigh_filtrado <- enigh_filtrado %>%
  mutate(est_socio2 = as.numeric(est_socio == 2),
         est_socio3 = as.numeric(est_socio == 3),
         est_socio4 = as.numeric(est_socio == 4))

library(lmtest)
library(sandwich)

enigh_filtrado <- na.omit(enigh_filtrado)
model1 <- lm(prop_alimen ~ est_socio2 + est_socio3 + est_socio4 + urbano + tot_integ + prop_mujeres + prop_menores + prop_p65mas, data = enigh_filtrado)
model2 <- lm(prop_alimen ~ log(ing_cor)+ urbano + tot_integ + prop_menores + prop_p65mas + sexo_jefe, data = enigh_filtrado)
model3 <- lm(alimentos_pc ~  est_socio2 + est_socio3 + est_socio4 + urbano + tot_integ + prop_mujeres + prop_menores + prop_p65mas, data = enigh_filtrado)
model4 <- lm(alimentos_pc ~ log(ing_cor)+ urbano + tot_integ + prop_menores + prop_p65mas + sexo_jefe, data = enigh_filtrado)
model5 <- lm(log(alimentos_pc) ~ est_socio2 + est_socio3 + est_socio4 + urbano + tot_integ + prop_mujeres + prop_menores + prop_p65mas, data = enigh_filtrado)
model6 <- lm(log(alimentos_pc) ~ log(ing_cor)+ urbano + tot_integ + prop_menores + prop_p65mas + sexo_jefe, data = enigh_filtrado)
